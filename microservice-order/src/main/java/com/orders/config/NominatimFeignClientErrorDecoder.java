        ...
        return switch (response.status()){
            ...
            default -> {
                try {
                    ...
                    log.error("Nominatim invalid response with status code {}: {}", response.status(), bodyString);
                } catch (IOException e) {
                    log.error("Error reading response body", e);
                } finally {
                    yield new BadGatewayException("Nominatim invalid response with status code " + response.status());
                }
            }
        };
    }
